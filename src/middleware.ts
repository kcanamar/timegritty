    ...
    async ({ cookies, url, locals }, next) => {
        ...
        switch (validationResult.status) {
            case 'authorized':
                ...
                try {
                    const session =
                        await sql`SELECT * FROM user_sessions WHERE auth_token = ${token?.value}`
                    const user =
                        await sql`SELECT u.name, u.id, u.password_hash, u.email FROM "user" u WHERE u.id = ${session[0].user_id}`
                    if (user) {
